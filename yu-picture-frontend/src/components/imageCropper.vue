  ...
  const applyEditAction = (action: string) => {
    switch (action) {
      ...
      case PICTURE_EDIT_ACTION_ENUM.ZOOM_OUT:
        ...
        break;
      default:
        console.warn('未知编辑操作:', action);
    }
  };
  // 本地点击时调用
  const rotateLeft = () => {
    applyEditAction(PICTURE_EDIT_ACTION_ENUM.ROTATE_LEFT);
    editAction(PICTURE_EDIT_ACTION_ENUM.ROTATE_LEFT); // 发送网络消息
  };

// SYNC_HISTORY 中调用
  websocket.on(PICTURE_EDIT_MESSAGE_TYPE_ENUM.SYNC_HISTORY, (msg) => {
    if (Array.isArray(msg.operations)) {
      msg.operations.forEach(op => applyEditAction(op.editAction));
    }
  });
  // 监听退出编辑状态消息
...
const enterEdit = () => {
  if (websocket) {
    ...
    // editingUser.value = loginUserStore.loginUser
  }
}
...
watchEffect(() => {
  console.log('loginUser:', loginUserStore.loginUser)
  console.log('editingUser:', editingUser.value)
  console.log('canExitEdit:', canExitEdit.value)
  console.log('canEdit:', canEdit.value)
  console.log('canEnterEdit:', canEnterEdit.value)
  console.log("isTeamSpace", isTeamSpace.value)
})
</script>
