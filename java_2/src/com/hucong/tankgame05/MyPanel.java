...
//    Enemy enemy03 = new Enemy(160, 0);
    ...
    public MyPanel(int key) {
        //判断文件是否存在 存在正常运行 不存在 只能开启新游戏
        File file = new File(Recorder.getRecordFile());
        if (file.exists()){
            nodes = Recorder.getNodesAndEnemiesRec();
        }else {
            System.out.println("文件不存在 请开新游戏");
            key = 1;
        }
        switch (key) {
            case 1:
                ...
                case 2:
                    ...
                    for (int i = 0; i < nodes.size(); i++) {
                        //先创建敌人对象
                        Enemy enemy = new Enemy(nodes.get(i).getX(), nodes.get(i).getY());
                        //将enemies设置给enemy
                        enemy.setEnemies(enemies);
                        //在定义好方向
                        enemy.setDirect(nodes.get(i).getDirect());
                        //启动敌人坦克线程 让其动起来
                        new Thread(enemy).start();
                        //给敌方加入一颗子弹
                        Shot shot = new Shot(enemy.getX() + 20, enemy.getY() + 60, enemy.getDirect());
        ...
        //播放指定的音乐
        new AePlayWave("src/com/hucong/tankgame05/Hope.wav").start();
    }
    ...
    public void hitEnemy(Shot s, Tank enemy) throws IOException {
